    ...
    for (int i = 0; i < 255; i++) {
...

        {
            ...
            ASSERT_THROW(decrypter.decrypt(ciphertext, b.get_epk(), uint256(), (i == 0) ? 1 : (i - 1)),
                         libzero::note_decryption_failed);
...
{
    for (size_t i = 0; i < 100; i++) {
        uint252 a_sk = libzero::random_uint252();
        uint256 rest;
        ASSERT_TRUE(
...
TEST(noteencryption, prf_nf)
{
    for (size_t i = 0; i < 100; i++) {
        uint252 a_sk = libzero::random_uint252();
        uint256 rho = libzero::random_uint256();
        ASSERT_TRUE(
...
TEST(noteencryption, prf_pk)
{
    for (size_t i = 0; i < 100; i++) {
        uint252 a_sk = libzero::random_uint252();
        uint256 h_sig = libzero::random_uint256();
        ASSERT_TRUE(
...
{
    for (size_t i = 0; i < 100; i++) {
        uint252 phi = libzero::random_uint252();
        uint256 h_sig = libzero::random_uint256();
        ASSERT_TRUE(
