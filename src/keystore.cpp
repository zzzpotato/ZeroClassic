...
bool CBasicKeyStore::AddSpendingKey(const libzero::SpendingKey &sk)
{
...
bool CBasicKeyStore::AddViewingKey(const libzero::ViewingKey &vk)
{
    LOCK(cs_SpendingKeyStore);
...
bool CBasicKeyStore::RemoveViewingKey(const libzero::ViewingKey &vk)
{
    LOCK(cs_SpendingKeyStore);
...
bool CBasicKeyStore::HaveViewingKey(const libzero::PaymentAddress &address) const
{
    LOCK(cs_SpendingKeyStore);
    return mapViewingKeys.count(address) > 0;
}

bool CBasicKeyStore::GetViewingKey(const libzero::PaymentAddress &address,
                                   libzero::ViewingKey &vkOut) const
{
    LOCK(cs_SpendingKeyStore);
