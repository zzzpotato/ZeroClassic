...
double benchmark_create_joinsplit()
{
    ...
    assert(jsdesc.Verify(*pzeroParams, verifier, pubKeyHash));
    return ret;
}
...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 pubKeyHash;
    auto verifier = libzero::ProofVerifier::Strict();
    joinsplit.Verify(*pzeroParams, verifier, pubKeyHash);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    for (int i = 0; i < nAddrs; i++) {
        auto sk = libzero::SpendingKey::random();
        wallet.AddSpendingKey(sk);
    }

    auto sk = libzero::SpendingKey::random();
    auto tx = GetValidReceive(*pzeroParams, sk, 10, true);
...
double benchmark_increment_note_witnesses(size_t nTxs)
{
    ...
    for (int i = 0; i < nTxs; i++) {
        auto wtx = GetValidReceive(*pzeroParams, sk, 10, true);
        auto note = GetNote(*pzeroParams, sk, wtx, 0, 1);
