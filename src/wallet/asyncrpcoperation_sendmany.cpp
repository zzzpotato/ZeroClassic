...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            auto hSig = prevJoinSplit.h_sig(*pzeroParams, tx_.joinSplitPubKey);
            try {
        ...
        uint256 anchor)
{
            ...
            &esk); // parameter expects pointer to esk, so pass in address
    {
        ...
        if (!(jsdesc.Verify(*pzeroParams, verifier, joinSplitPubKey_))) {
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pzeroParams, joinSplitPubKey_);
    ...
    for (int i = 0; i < ZC_NUM_JS_OUTPUTS; i++) {
        ...
        libzero::PaymentAddress zaddr = output.addr;  // randomized output
