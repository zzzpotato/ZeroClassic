...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"zeroprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"zeroprivkey\"   (string, required) The private key (see dumpprivkey)\n"
    ...
    while (file.good()) {
        ...
        if (fImportZKeys) {
            try {
                ...
                libzero::SpendingKey key = spendingkey.Get();
                libzero::PaymentAddress addr = key.address();
                ...
                // Not a valid spending key, so carry on and see if it's a Zero style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"zeroaddress\"\n"
            "\nReveals the private key corresponding to 'zeroaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"zeroaddress\"   (string, required) The zero address for the private key\n"
            "\nResult:\n"
