...
bool CWallet::AddZKey(const libzero::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libzero::PaymentAddress &address,
...
bool CWallet::LoadCryptedZKey(const libzero::PaymentAddress &addr, const libzero::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libzero::SpendingKey &key)
{
    return CCryptoKeyStore::AddSpendingKey(key);
}

bool CWallet::AddViewingKey(const libzero::ViewingKey &vk)
{
...
bool CWallet::RemoveViewingKey(const libzero::ViewingKey &vk)
{
...
bool CWallet::LoadViewingKey(const libzero::ViewingKey &vk)
{
                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                            *pzeroParams, wtxItem.second.joinSplitPubKey);
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pzeroParams, tx.joinSplitPubKey);
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent, bool ignoreUnspendable)
{
    ...
    libzero::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pzeroParams, wtx.joinSplitPubKey);
            try {
