...
CWalletTx GetValidReceive(const libzero::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libzero::Note GetNote(const libzero::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libzero::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libzero::SpendingKey::random();
    auto sk2 = libzero::SpendingKey::random();
