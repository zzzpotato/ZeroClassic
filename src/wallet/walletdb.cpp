...
bool CWalletDB::WriteCryptedZKey(const libzero::PaymentAddress & addr,
...
bool CWalletDB::WriteZKey(const libzero::PaymentAddress& addr, const libzero::SpendingKey& key, const CKeyMetadata &keyMeta)
{
    nWalletDBUpdated++;
...
bool CWalletDB::WriteViewingKey(const libzero::ViewingKey &vk)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("vkey"), vk), '1');
}

bool CWalletDB::EraseViewingKey(const libzero::ViewingKey &vk)
{
    nWalletDBUpdated++;
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        else if (strType == "czkey")
        {
            ...
            uint256 rkValue;
            ssValue >> rkValue;
            libzero::ReceivingKey rk(rkValue);
