...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libzero::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libzero::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libzero::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libzero::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libzero::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libzero::PaymentAddress &address, libzero::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libzero::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libzero::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
