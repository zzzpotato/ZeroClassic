...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ZeroClassicMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL))
        return error("ZeroClassicMiner: ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ZeroClassicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zeroclassic-miner");
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in ZeroClassicMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in ZeroClassicMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running ZeroClassicMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ZeroClassicMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ZeroClassicMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
