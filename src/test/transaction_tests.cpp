...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libzero's job to ensure the
    // integrity of the scheme through its own tests.

    // construct a merkle tree
    ZCIncrementalMerkleTree merkleTree;

    libzero::SpendingKey k = libzero::SpendingKey::random();
    libzero::PaymentAddress addr = k.address();

    libzero::Note note(addr.a_pk, 100, uint256(), uint256());
    ...
    boost::array<libzero::JSInput, ZC_NUM_JS_INPUTS> inputs = {
        libzero::JSInput(witness, note, k),
        libzero::JSInput() // dummy input of zero value
    };
    boost::array<libzero::JSOutput, ZC_NUM_JS_OUTPUTS> outputs = {
        libzero::JSOutput(addr, 50),
        libzero::JSOutput(addr, 50)
    };
    ...
    {
        JSDescription jsdesc(*pzeroParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pzeroParams, verifier, pubKeyHash));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pzeroParams, verifier, pubKeyHash));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(*pzeroParams, pubKeyHash, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(*pzeroParams, pubKeyHash, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(*pzeroParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pzeroParams, verifier, pubKeyHash));
    }
}
