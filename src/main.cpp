...
#if defined(NDEBUG)
# error "Zero cannot be compiled without assertions."
#endif
                      ...
                      libzero::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*pzeroParams, verifier, tx.joinSplitPubKey)) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    auto verifier = libzero::ProofVerifier::Strict();
    auto disabledVerifier = libzero::ProofVerifier::Disabled();
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In Zero this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see Zero protocol spec
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libzero::ProofVerifier::Disabled();
