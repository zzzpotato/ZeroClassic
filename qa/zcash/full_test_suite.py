#!/usr/bin/env python2
#
# Execute all of the automated tests related to Zero.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/zero/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/zero/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/zero/GenerateParams')
    ...
    ret &= test_fortify_source('src/zero/GenerateParams')
