...
Jack Grigg (22):
      ...
      Instruct users to run zero-fetch-params if network params aren't available
      ...
      Throw an error if zero.conf is missing
      Show a friendly message explaining why zerod needs a zero.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty zero.conf during performance measurements
      Create empty zero.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from zero.conf to avoid problems with command line tools
