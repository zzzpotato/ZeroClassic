...
Backing up your Zero private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
These instructions are specific for the officially supported Zero Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`zero.conf` located in the data directory which is `~/.zeroclassic/` unless it's been overridden with `datadir=` setting):
...
If you generate a new Zero address, it will not be reflected in the backup file.
