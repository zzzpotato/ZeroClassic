...
#Run with NO_PROTON=1 ./zcutil/build-win.sh --disable-rust
...
cd depends/ && make HOST=$HOST V=1 NO_QT=1 NO_RUST="$RUST_FLAG"
cd "$(dirname "$(readlink -f "$0")")/.." 
...
cd src/
CC="${CC}" CXX="${CXX}" make V=1 zerod.exe zero-cli.exe zero-tx.exe
